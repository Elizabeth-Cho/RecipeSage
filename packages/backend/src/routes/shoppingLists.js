  ...
  wrapRequestWithErrorHandler(async (req, res) => {
    const shoppingList = await SQ.transaction(async (transaction) => {
      ...
      if (req.body.collaborators?.length) {
        await shoppingList.addCollaborators(
...
router.post(
  ...
  wrapRequestWithErrorHandler(async (req, res) => {
    ...
    if (!shoppingList) {
      throw NotFound('Shopping list with that ID not found or you do not have access!');
    }

    await ShoppingListItem.bulkCreate(req.body.items.map((item) => ({
      title: item.title,
